class Sudoku
  ...
  def []=(x, y, value)
    raise "#{value} не допустиме у строці #{y}" unless allowed_in_row(y).include?(value)
    raise "#{value} не допустиме в колонці #{x}" unless allowed_in_column(x).include?(value)
    raise "#{value} не допустиме в квадраті у #{x}, #{y}" unless allowed_in_square(x, y).include?(value)
    @board[y][x] = value
  end
